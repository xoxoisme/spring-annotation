...
- 사전적 의미는 **주석**이지만, 자바에서는 코드 사이에 특별한 의미, 기능을 수행하도록 하는 기술이다.
...
#### @ManyToOne
- 해당 필드가 다른 엔티티를 참조할 때 사용한다.
- 보통 `EAGER`가 기본 적용되어 있으나, 성능을 위해 `LAZY`를 지향한다.
- 연관관계의 테이블을 조회해야할 때만 쿼리가 나가도록 지연로딩을 하는 것이 성능에 좋다.
